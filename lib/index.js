...
I18N.prototype.setLocale = function( localeData ) {
	if ( localeData && localeData[ '' ] && localeData[ '' ][ 'key-hash' ] ) {
		let l, minHashLength, maxHashLength;
		...
		if ( keyHash.substr( 0, 4 ) === 'sha1' ) {
			if ( keyHash.length === 4 ) {
				...
				if ( variableHashLengthPos < 0 ) {
					translationLookup.push( generateLookup( Number( keyHash.substr( 5 ) ) ) );
				} else {
					...
					for ( l = minHashLength; l <= maxHashLength; l++ ) {
						translationLookup.push( generateLookup( l ) );
					}
				}
			}
		}
 ...
 * @param {object} localeData The jed locale object
 */
...
I18N.prototype.translate = function() {
	let translation, sprintfArgs, errorMethod, optionsString;
